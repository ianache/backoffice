<template>
  <section class="bg-surface-container-low border-b border-outline-variant px-lg py-md flex flex-wrap items-center justify-between gap-md">
    <!-- Selector de Contexto (RF-01) -->
    <div class="flex flex-wrap items-center gap-md">
      <div class="flex items-center gap-xs">
        <span class="material-symbols-outlined text-primary">layers</span>
        <span class="font-label-sm uppercase tracking-wider text-on-surface-variant">Contexto de Resolución:</span>
      </div>
      <!-- Tenant Selector -->
      <div class="flex flex-col">
        <label class="text-[10px] text-on-surface-variant uppercase font-bold">Tenant</label>
        <select
          id="tenantSelect"
          v-model="selectedTenantId"
          :disabled="!isPlatformAdmin"
          class="bg-surface border border-outline-variant rounded px-sm py-1 font-body-md text-sm text-on-surface focus:ring-primary focus:border-primary disabled:opacity-60"
        >
      ...
      <span class="text-on-surface-variant mt-3 font-black">/</span>
      <!-- Company Selector -->
      <div class="flex flex-col">
        <label class="text-[10px] text-on-surface-variant uppercase font-bold">Company (Override N2)</label>
        <select
          id="companySelect"
          v-model="selectedCompanyId"
          class="bg-surface border border-outline-variant rounded px-sm py-1 font-body-md text-sm text-on-surface focus:ring-primary focus:border-primary"
        >
      ...
      <!-- Product Selector -->
      <div class="flex flex-col">
        <label class="text-[10px] text-on-surface-variant uppercase font-bold">Product (Override N3)</label>
        <select
          id="productSelect"
